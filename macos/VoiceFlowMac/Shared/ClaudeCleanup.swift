...
struct ClaudeCleanup {
    ...
    func clean(_ request: Request) async throws -> String {
        ...
        // A single word doesn't need AI cleanup and sending a huge
        // system prompt for "hello" is wasteful + triggers
        // conversational responses from the model. Just return it.
        let wordCount = trimmed.split(whereSeparator: \.isWhitespace).count
        if wordCount <= 2 && request.spliceContext == nil {
            return trimmed
        }

        // Wrap the transcript in <transcript> tags so Claude cannot
        // interpret the content as a conversational message. This is
        // the primary defense against the model responding with
        // "I'm ready to clean up your dictation..." instead of
    ...
    greet the user. You do NOT say "I'm ready" or "paste your \
    text" or "here's the cleaned version". You are a text \
    processor: transcript goes in, cleaned text comes out. Nothing \
    else.

    The user is holding their phone, waiting for text they can \
    immediately paste into Outlook, Teams, Slack, Mail, Notes, \
    Salesforce, a document, a DM — somewhere real. They want to \
    hit Cmd-V and be done. Your output IS the final text.
    ...
    private static func looksLikePromptEcho(_ output: String) -> Bool {
        ...
        // Hard signals: ANY ONE of these means it's definitely an
        // echo/conversational response, not cleaned transcript.
        ...
        // Soft signals: 2+ of these together mean echo.
